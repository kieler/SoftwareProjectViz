...
/**
 * This class is a command line tool that executes the generation of the Yarn + Inversify model files.
 * The project parameters for this generation are configured via command line parameters.
 *
 * @author nre
 */
@Command(name = "yarnInversifymodelgen")
public class ConfigAndExecuteCli implements Runnable {
	...
	public void run() {
		if (projectPaths != null) {
			...
			final Map<String, YarnInversifyProject> projectMap = new HashMap<String, YarnInversifyProject>();
			for (final Entry<String, File> projectPathEntry : projectPaths.entrySet()) {
				LOGGER.log(System.Logger.Level.INFO, "Reading Project Data for " + projectNames.get(projectPathEntry.getKey()));
				final String projectKey = projectPathEntry.getKey();
				projectMap.put(projectKey, YarnInversifyModelDataGenerator.generateData(projectPathEntry.getValue().toString(),
						projectNames.get(projectKey), optionalModelSaveFilePath));
			}

			LOGGER.log(System.Logger.Level.INFO, "YarnInversify model generation has finished. The files can be found in " + modelSaveFilePath);
		}
	}
